class Events::ParticipationsController < Events::ApplicationController
  ...
  def create
    current_user.participations.find_or_create_by!(event: @event) # お気に入り済みの場合は例外を吐かないように
    redirect_to event_path(@event), notice: t('controllers.created')
  end

  def destroy
    participation = current_user.participations.find_by(event: @event) # windowを複数開いてお気に入りを解除した時に例外を吐かないように
